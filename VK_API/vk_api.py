...
col_names = ['Full Name', 'Sex', 'VK_id', 'Birth Date', 'Relation', 'Languages']
...
find_users = requests.get(search_url, params=search_params)            #находим 200 человек из города Белокуриха

data = find_users.json()

fields = {'first_name':0, 'last_name':0, 'sex':1,'uid':2, 'bdate':3, 'relation':4,'langs':5}

for i in range(1, len(data['response'])):
    metadata = [''] * 6
    for feature in data['response'][i]:                                         #ищем интересующие нас социологические данные по пользователям
        if feature in fields.keys():
            metadata[fields[feature]] += ' ' + str(data['response'][i][feature])
            metadata[fields[feature]] = metadata[fields[feature]].strip()
        if feature == 'personal':
            try:
                metadata[fields['langs']] = data['response'][i][feature]['langs']
            except:
                pass
    ...
    get_wall = requests.get(wall_url, params=wall_params)             #выкачиваем записи со стены пользователя через id
    posts = get_wall.json()
    with open('VK_Belokurikha\Posts_of_' + str(target_id) + '.txt', 'a', encoding='utf-8') as current_file:        #записываем посты в файлы
        try:
