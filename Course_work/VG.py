class VerbGovernment:
    ...
    def pmi_preps(self):
        ...
        for index, _ in self.table[self.table.lemma == self.verb].iterrows():
            if self.table['gramm'].loc[index][2] not in 'mpgn':
                words = 0
                left = 1
                while self.table['link'].loc[
                        index -
                        left] != 'PUNC' and words < 5 and index - left >= 0:
                    words += 1
                    current_word = self.table['lemma'].loc[index - left]
                    if self.table['POS'].loc[index - left] == 'S':
                        prepositions[current_word] = prepositions.get(
                            current_word, 0) + 1
                    left += 1
                ...
                while self.table['link'].loc[
                        index +
                        right] != 'PUNC' and words < 5 and index + right < len(
                            self.table):
                    words += 1
        ...
        prep_pmi = dict()
        words_number = len(self.table[(self.table['link'] != 'PUNC') & (
            self.table['head'] != 'PUNC')])
        verb_freq = len(self.table[self.table.lemma ==
                                   self.verb]) / words_number
        margin = statistics.mean(prepositions.values())

        for prep in prepositions.keys():
            if prepositions[prep] < margin:
                continue
            prep_freq = len(self.table[self.table.lemma ==
                                       prep]) / words_number
    ...
    def get_config(self, tags, tokens, pmi_preps):
        ...
        for (tag, token) in zip(tags, tokens):
            if tag == '' or tag[0] in 'RAI' or tag[0] == 'V' and tag[
                    2] in 'mpg':
                continue
                ...
                if tag[0] == 'M':
                    ...
                    has_numeral = True
                if tag[0] in 'NP' and self.get_case(
                        tag
                ) == case:  #удаляем, т.к. входит в предложную группу, такой же падеж
                    continue
                #удаляем указательные местоимение, т.к. такой же падеж, как и у сущ. перед ним
                if len(config) != 0 and config[-1][0] == 'P' and tag[
                        0] == 'N' and config[-1][1] == self.get_case(tag):
                    config[-1] = 'N' + self.get_case(tag)
                new_case = num_case if tag[
                    0] == 'N' and has_numeral else self.get_case(tag)
    ...
    def extractor(self, verb, table):
        ...
        for index, _ in table[table.lemma == self.verb].iterrows():
            if table['gramm'].loc[index][2] not in 'mpgn':
                ...
                while table['lemma'].loc[index - left] not in [
                        ',', '-', ':', '.', '!', '?', ';'
                ] and self.table['POS'].loc[
                        index -
                        left] != 'C' and words < 5 and index - left >= 0:
                    if re.search('\w', table['token'].loc[index - left]):
                        ...
                        if tags[5 - words][0] in 'PN' and self.get_case(tags[
                                5 - words]) == 'g' and change_gen_left:
                            change_gen_left = False
                            if tags[5 - words][0] == 'N':
                                tags[5 - words] = tags[
                                    5 - words][:4] + 'a' + tags[5 - words][5:]
                            else:
                                tags[5 - words] = tags[
                                    5 - words][:5] + 'a' + tags[5 - words][6:]
                    left += 1

                right = 1
                words = 0
                while table['lemma'].loc[index + right] not in [
                        ',', '-', ':', '.', '!', '?', ';'
                ] and (self.table['POS'].loc[index + right - 1] != 'V' or right
                       == 1) and words < 5 and index + right < len(table):
                    if re.search('\w', table['token'].loc[index + right]):
                        ...
                        if tags[5 + words][0] in 'NP' and self.get_case(tags[
                                5 + words]) == 'g' and change_gen_right:
                ...
                if table['gramm'].loc[index +
                                      right] == ',' and words < 5 and table[
                                          'gramm'].loc[index + right + 1] in [
                                              'что', 'как', 'чтобы', 'когда'
                                          ]:
                    tags[6 + words] = table['lemma'].loc[index + right + 1]
                    tokens[6 + words] = table['token'].loc[index + right + 1]

                current_config = self.combiner(
                    self.get_config(tags, tokens, self.possible_preps))
                if 'S-' not in current_config:
                    self.configs.append(current_config)
                    config_str = ' '.join(current_config)
                    if (config_str not in self.examples or
                            len(self.examples[config_str]) < 3
                        ) and sentence_id != table.sent_id.loc[index]:
                        sentence_id = table.sent_id.loc[index]
                        self.examples[config_str] = self.examples.get(
                            config_str,
                            []) + [self.get_sent(table, sentence_id)]
...
#         return configs
    ...
    def government(self):
        ...
        for chain in sorted(config_freq, key=config_freq.get, reverse=True):
            final_result.append(
                (chain, config_freq[chain], self.examples[chain]))
...
for num in range(1, 5):
    document = pd.read_csv(
        '.\corpora\corpus-i-part' + str(num) + '.txt',
        delimiter='\t',
        header=0)
